...
export async function getCollectionPage(
  ...
  const cards: CollectionCard[] = (data ?? []).map((row) => {
    ...
    const userCard = Array.isArray(uc)
      ? uc[0]
      : (uc as unknown as { stars: number; obtained_at: string })
 ...
 */
export async function getAlbumsWithProgress(): Promise<AlbumWithProgress[] | { error: string }> {
  const supabase = await createClient()
  ...
  return (albums ?? []).map((album) => {
    ...
    const completionPct =
      cardIds.length > 0 ? Math.round((collectedIds.length / cardIds.length) * 100) : 0
